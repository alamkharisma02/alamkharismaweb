...
<div class="max-w-2xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="flex items-center space-x-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Ubah Testimoni</h1>
            <p class="text-sm text-slate-500">Ubah detail ulasan dari {{ $testimonial->name }}.</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.testimonials.update', $testimonial->id) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $testimonial->name) }}" required placeholder="Contoh: David Miller"
                ...
                <div>
                    ...
                    <input type="text" name="company" id="company" value="{{ old('company', $testimonial->company) }}" required placeholder="Contoh: Pemilik Villa Canggu"
                           class="block w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent bg-slate-50 text-slate-900 font-semibold">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="avatar" id="avatar" value="{{ old('avatar', $testimonial->avatar) }}" placeholder="Contoh: DM" max="10"
                           class="block w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent bg-slate-50 text-slate-900 font-semibold">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="5" required placeholder="Tuliskan ulasan kepuasan klien di sini..."
                          class="block w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent bg-slate-50 text-slate-900 leading-relaxed">{{ old('content', $testimonial->content) }}</textarea>
            </div>
