...
<div class="max-w-4xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="flex items-center space-x-3">
        ...
        <div>
            ...
            <p class="text-sm text-slate-500">Daftarkan portofolio pembangunan sipil baru di website.</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="Contoh: Pembangunan Villa Tropis Bali"
                ...
                <div>
                    ...
                    <input type="text" name="location" id="location" value="{{ old('location') }}" required placeholder="Contoh: Badung, Bali"
                           class="block w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="category" id="category" required class="block w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent">
                        <option value="Sipil" {{ old('category') == 'Sipil' ? 'selected' : '' }}>Pekerjaan Sipil / Struktur</option>
                        <option value="Interior" {{ old('category') == 'Interior' ? 'selected' : '' }}>Pekerjaan Interior / Furniture</option>
                        <option value="Eksterior" {{ old('category') == 'Eksterior' ? 'selected' : '' }}>Pekerjaan Eksterior / Fasad</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" required class="block w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent">
                        <option value="Planning" {{ old('status') == 'Planning' ? 'selected' : '' }}>Planning / Perencanaan</option>
                        <option value="In Progress" {{ old('status') == 'In Progress' ? 'selected' : '' }}>In Progress / Sedang Dibangun</option>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="url" name="video_url" id="video_url" value="{{ old('video_url') }}" placeholder="Contoh: https://www.youtube.com/embed/dQw4w9WgXcQ"
                           class="block w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="5" placeholder="Tuliskan spesifikasi pengerjaan konstruksi, material utama, dll..."
                          class="block w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent">{{ old('description') }}</textarea>
            </div>
