...
<div class="max-w-4xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="flex items-center space-x-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Ubah Artikel</h1>
            <p class="text-sm text-slate-500">Ubah konten artikel "{{ $article->title }}".</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $article->title) }}" required placeholder="Contoh: Mengenal Mutu Beton ReadyMix vs SiteMix"
                ...
                <div>
                    ...
                    <select name="category" id="category" required class="block w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent">
                        <option value="Tips & Panduan" {{ old('category', $article->category) == 'Tips & Panduan' ? 'selected' : '' }}>Tips & Panduan</option>
                        <option value="Teknologi Material" {{ old('category', $article->category) == 'Teknologi Material' ? 'selected' : '' }}>Teknologi Material</option>
                        <option value="Info Sipil" {{ old('category', $article->category) == 'Info Sipil' ? 'selected' : '' }}>Info Sipil</option>
                        <option value="Inspirasi Desain" {{ old('category', $article->category) == 'Inspirasi Desain' ? 'selected' : '' }}>Inspirasi Desain</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label for="cover_file" class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Ganti Gambar Sampul (Cover)</label>
                        <input type="file" name="cover_file" id="cover_file" accept="image/*"
                               onchange="previewImage(this, 'cover-preview-container', 'cover-preview-img'); if (this.files && this.files[0]) { document.getElementById('current-cover-container')?.classList.add('hidden'); } else { document.getElementById('current-cover-container')?.classList.remove('hidden'); }"
                    ...
                    <div id="cover-preview-container" class="space-y-1.5 hidden">
                        <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Preview Gambar Baru:</span>
                        <div class="w-32 h-20 rounded-lg overflow-hidden border border-slate-200 bg-slate-100">
                            <img id="cover-preview-img" class="w-full h-full object-cover" alt="New Cover Preview">
                        </div>
                    </div>
                    @if($article->cover_image)
                        <div id="current-cover-container" class="space-y-1.5">
                            <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Sampul Saat Ini:</span>
                            <div class="w-32 h-20 rounded-lg overflow-hidden border border-slate-200 bg-slate-100">
                                <img src="{{ $article->cover_image }}" class="w-full h-full object-cover" alt="Current Cover">
                            </div>
                        </div>
                    @endif
                </div>
            ...
            <div>
                ...
                <input type="url" name="external_link" id="external_link" value="{{ old('external_link', $article->external_link) }}" placeholder="Contoh: https://example.com/artikel (Jika diisi, pembaca akan langsung diarahkan ke link ini)"
                       class="block w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent">
            </div>
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="10" placeholder="Tuliskan isi pembahasan..."
                          class="block w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent">{{ old('content', $article->content) }}</textarea>
            </div>
