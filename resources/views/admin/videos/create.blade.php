...
<div class="max-w-2xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="flex items-center space-x-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Tambah Video Dokumentasi</h1>
            <p class="text-sm text-slate-500">Masukkan detail video dokumentasi proyek atau profil perusahaan Anda.</p>
        </div>
    </div>
    ...
    <!-- Form -->
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8" x-data="{ videoSource: '{{ old('video_file') ? 'upload' : 'youtube' }}' }">
        <form action="{{ route('admin.videos.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="Contoh: Proses Pengecoran Dak Beton Lantai 2"
            ...
            <div>
                <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Sumber Video*</label>
            ...
            <div x-show="videoSource === 'youtube'" x-transition>
                ...
                <input type="url" name="video_url" id="video_url" value="{{ old('video_url') }}" placeholder="Contoh: https://www.youtube.com/watch?v=yGcwSC8Z6_c"
                       class="block w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent bg-slate-50 text-slate-900 font-semibold">
            ...
            <div x-show="videoSource === 'upload'" x-transition style="display: none;">
                <label for="video_file" class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">File Video Lokal* (MP4/WebM/QuickTime)</label>
                ...
                <p class="text-[10px] text-slate-400 mt-1">*Maksimum ukuran berkas: 100MB. Format yang disarankan: .mp4 atau .webm</p>
            </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" placeholder="Tuliskan deskripsi singkat mengenai aktivitas proyek pada video ini..."
                          class="block w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent bg-slate-50 text-slate-900 leading-relaxed">{{ old('description') }}</textarea>
            </div>
